...
def pack_audio(cfg, audio_record, temporal_sample_index):
    path_audio = os.path.join(cfg.URBANSOUND8K.AUDIO_DATA_FILE, 'fold' + str(audio_record['fold']), str(audio_record['video']))
    import librosa
    samples, sr = librosa.core.load(path_audio, sr=cfg.AUDIO_DATA.SAMPLING_RATE, mono=True)
...
def _extract_sound_feature(cfg, samples, start_idx, end_idx):
    if samples.shape[0] < int(round(cfg.AUDIO_DATA.SAMPLING_RATE * cfg.AUDIO_DATA.CLIP_SECS)):
        import librosa
        samples = librosa.util.fix_length(samples, size=int(round(cfg.AUDIO_DATA.SAMPLING_RATE * cfg.AUDIO_DATA.CLIP_SECS))-1, mode='edge')
