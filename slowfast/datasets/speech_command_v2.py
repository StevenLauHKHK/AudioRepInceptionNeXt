...
from .audio_loader_speechcommandv2 import pack_audio

logger = logging.get_logger(__name__)


@DATASET_REGISTRY.register()
class Speechcommandv2(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode):
        ...
        assert mode in [
            ...
            "test",
            "train+val"
        ], "Split '{}' not supported for SPEECH_CMD_V2".format(mode)
        self.cfg = cfg
        self.mode = mode
        ...
        logger.info("Constructing Speech Command V2 Audio {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        """
        if self.mode == "train":
            path_annotations_pickle = [os.path.join(self.cfg.SPEECHCOMMANDV2.ANNOTATIONS_DIR, self.cfg.SPEECHCOMMANDV2.TRAIN_LIST)]
        elif self.mode == "val":
            path_annotations_pickle = [os.path.join(self.cfg.SPEECHCOMMANDV2.ANNOTATIONS_DIR, self.cfg.SPEECHCOMMANDV2.VAL_LIST)]
        elif self.mode == "test":
            path_annotations_pickle = [os.path.join(self.cfg.SPEECHCOMMANDV2.ANNOTATIONS_DIR, self.cfg.SPEECHCOMMANDV2.TEST_LIST)]
        else:
            path_annotations_pickle = [os.path.join(self.cfg.SPEECHCOMMANDV2.ANNOTATIONS_DIR, file)
                                       for file in [self.cfg.SPEECHCOMMANDV2.TRAIN_LIST, self.cfg.SPEECHCOMMANDV2.VAL_LIST]]
