...
def test(cfg):
    ...
    test_checkpoint_epoch = cu.load_test_checkpoint(cfg, model)

    if cfg.MODEL.MERGE_MODE:
        logger.info("Merge the kernel")
        model.module.merge_kernel()

    if cfg.MODEL.OUTPUT_DIR != "":
        logger.info("Saving the merged kernel")
        cu.save_checkpoint(cfg.MODEL.OUTPUT_DIR, model, None, test_checkpoint_epoch, cfg)
